...
namespace StringWorkAndElectricityClass
{
    internal class AccountingElectricity
    {
        ...
        static public AccountingElectricity operator+(AccountingElectricity accounting1, AccountingElectricity accounting2)
        {
            AccountingElectricity res = new AccountingElectricity();
            res.accounts = new List<KeyValuePair<string, int>>(accounting1.accounts);
            res.accounts.AddRange(accounting2.accounts);
            res.electricity = new Information[accounting1.countApartment + accounting2.countApartment, 3];

            for(int i = 0; i < accounting1.countApartment; i++)
            {
                for(int j = 0; j < 3; j++)
                {
                    res.electricity[i, j] = accounting1.electricity[i, j];
                }
            }

            for (int i = 0; i < accounting2.countApartment; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    res.electricity[i+accounting1.countApartment, j] = accounting2.electricity[i, j];
                }
            }
        ...
        static public AccountingElectricity operator -(AccountingElectricity accounting1, AccountingElectricity accounting2)
        {
            AccountingElectricity res = new AccountingElectricity();
            res.accounts = new List<KeyValuePair<string, int>>(accounting1.accounts);
            List<int> indexOfDoNotRemoveApartment = new List<int>();
            for(int i = 0; i < accounting1.CountApartment; i++)
            {
                for(int j = 0; j < accounting2.CountApartment; j++)
                {
                    if(accounting1.accounts[i].Key == accounting2.accounts[j].Key && accounting1.accounts[i].Value == accounting2.accounts[j].Value)
                    {
                        res.accounts.Remove(accounting1.accounts[i]);
                    }
                    
                }
            }

            for(int i = 0; i< res.accounts.Count; i++)
            {
                indexOfDoNotRemoveApartment.Add(accounting1.accounts.IndexOf(res.accounts[i]));
            }

            res.electricity = new Information[res.accounts.Count, 3];
            ...
            for (int i = 0; i < res.accounts.Count; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    res.electricity[i, j] = accounting1.electricity[indexOfDoNotRemoveApartment[i], j];
                }
            }
